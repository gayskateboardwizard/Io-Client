...
public final class IoUserCapeService {
    ...
    private static final String WORKER_URL = "https://io-users-worker.YOUR_WORKER_SUBDOMAIN.workers.dev/";
    private static final Set<String> ACTIVE_USERS = ConcurrentHashMap.newKeySet();
    private static final long POLL_INTERVAL_MS = Duration.ofMinutes(5).toMillis();
    ...
    public static void pollIfNecessary() {
        if (!isConfigured() || !isCapeFeatureEnabled() || requestInFlight)
            return;
        ...
        HTTP_CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                ...
                .whenComplete((res, ex) -> requestInFlight = false);
    }
        ...
        try {
            ...
            for (JsonElement e : usersArray) {
                if (e.isJsonPrimitive())
                    latest.add(e.getAsString().toLowerCase());
            }
            ACTIVE_USERS.clear();
